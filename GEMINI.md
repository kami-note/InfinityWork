...
- **Always invoke `global.gc()`** (using `setTimeout(() => global.gc?.(), 0)`) after processing large file uploads or stream proxying in `/api/files/upload/route.ts` and `/api/files/uploads/[id]/chunks/[index]/route.ts` to ensure memory is released immediately on low-memory VPS host environments.
- The portal Node.js process must be run with the `--expose-gc` flag (via `NODE_OPTIONS="--expose-gc"`).
